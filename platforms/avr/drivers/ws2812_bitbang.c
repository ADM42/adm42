...
void ws2812_setleds(rgb_led_t *ledarray, uint16_t number_of_leds) {
    ...
    // Measured single color => 528 - 28 (MCU) = 500mA, 500 / 42 = 11.9
    // WS28 12 means single color => 12mA
    // 255 / 12 = 21.25
    // Floating point arithmetic is slow and takes additional space in the firmware
    float total_rgb = 0;
    for (uint8_t i = 0; i < number_of_leds; i++) {
        total_rgb += ledarray[i].r + ledarray[i].g + ledarray[i].b;
    }
    if (total_rgb > max_ma * 22) {
        over_ma = true;
        // If total_rgb is not float f is not correct anymore...
        // Correct values before they are used by the driver
        float f = (max_ma * 22) / total_rgb;
        for (uint8_t i = 0; i < number_of_leds; i++) {
            ledarray[i].r *= f;
            ledarray[i].g *= f;
            ledarray[i].b *= f;
