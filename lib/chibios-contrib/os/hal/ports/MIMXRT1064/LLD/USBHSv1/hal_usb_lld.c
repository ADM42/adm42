 ...
 * @brief   MIMXRT1064 USB subsystem low level driver source.
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(MIMXRT1064_USB_OTG1_IRQ_VECTOR) {
  USBDriver *usbp = &USBD1;
  OSAL_IRQ_PROLOGUE();
...
void usb_lld_start(USBDriver *usbp) {
  ...
  nvicEnableVector(USB_OTG1_IRQn, MIMXRT1064_USB_USB1_IRQ_PRIORITY);
...
void usb_lld_init_endpoint(USBDriver *usbp, usbep_t ep) {
  ...
  if(ep > MIMXRT1064_USB_ENDPOINTS)
    return;
